        ...
        rows.append(
            {
                ...
                "mean_smape_mean_pct": float(group_frame["smape_mean_pct"].mean()),
        ...
        rows.append(
            {
                ...
                "mean_smape_mean_pct": row["mean_smape_mean_pct"],
...
def metric_guide() -> pd.DataFrame:
    return pd.DataFrame(
        [
            ...
            ("wape_pct", "100 * sum(abs(predetto - osservato)) / sum(abs(osservato)).", "Errore percentuale aggregato", "Instabile se la somma assoluta osservata e vicina a zero."),
            ("smape_mean_pct", "Media della sMAPE per cella: 100 * abs(errore) / ((abs(pred)+abs(obs))/2).", "Errore percentuale cella-per-cella", "Resta sensibile quando predetto e osservato sono entrambi molto piccoli."),
