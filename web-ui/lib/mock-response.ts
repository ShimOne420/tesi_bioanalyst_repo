...
export function createDemoResponse(input: {
  ...
  const monthly: IndicatorRow[] = months.map((month, index) => {
    ...
    return {
      ...
      precipitation_mean_daily_area_mm: Number(((48 + rainSeasonal * 22 + Math.abs(lonCenter) * 0.3) / 30).toFixed(2)),
      precipitation_unit: "mm/mese",
      ndvi_mean_area: Number((0.46 + seasonal * 0.12).toFixed(3)),
      swvl1_mean_area: Number((0.24 + rainSeasonal * 0.04).toFixed(3)),
      swvl2_mean_area: Number((0.28 + rainSeasonal * 0.035).toFixed(3)),
      cropland_mean_area: Number((0.34 + Math.abs(lonCenter) * 0.002).toFixed(3)),
      valid_cell_count: Math.round(areaFactor),
