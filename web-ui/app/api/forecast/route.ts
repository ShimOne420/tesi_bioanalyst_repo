...
export async function POST(request: Request) {
  try {
    ...
    if (!body.targetMonth) {
      return NextResponse.json({ error: "Mese target mancante." }, { status: 400 });
    }
    ...
    const forwarded = await fetch(`${backendBaseUrl}/api/forecast`, {
      method: "POST",
      headers: {
    ...
    return NextResponse.json(
      {
        error: error instanceof Error ? error.message : "Errore imprevisto nel backend forecast."
      },
      { status: 500 }
    );
  }
