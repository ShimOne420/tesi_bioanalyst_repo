...
export async function GET(request: Request, context: RouteContext) {
  try {
    ...
    const target = `${getBackendBaseUrl()}/api/forecast/cells/${label}?month=${encodeURIComponent(month)}`;
    ...
    return NextResponse.json(
      {
        error: error instanceof Error ? error.message : "Errore imprevisto nel recupero celle forecast."
      },
      { status: 500 }
    );
  }
